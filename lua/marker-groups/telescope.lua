---@class MarkerGroupsTelescope
---Telescope integration for marker-groups.nvim providing enhanced pickers
---for group selection and marker navigation with preview functionality.
---Includes graceful degradation when Telescope is not available.
local M = {}
...
if not has_telescope then
  -- Provide stub functions that notify user when Telescope is not available
  ---Show groups picker (stub function when Telescope unavailable)
  function M.show_groups()
    feedback.warning("Telescope Integration", 
      "Telescope is not available. Please install telescope.nvim to use this feature.")
  end
  
  ---Show markers picker (stub function when Telescope unavailable)
  ...
  ---@return boolean available False when Telescope is not installed
  function M.is_available()
    return false
  end
  
  ---Get status information about Telescope integration
  ---@return table status Status information including availability and reason
...
-- Load Telescope dependencies (only when Telescope is available)
...
---Check if Telescope integration is available
---@return boolean available True when Telescope is properly installed
function M.is_available()
  return true
end

---Get status information about Telescope integration
---@return table status Status information including availability and version
function M.get_status()
...
---Format date for display in previews
---@param timestamp number Unix timestamp
---@return string formatted_date Human-readable date string
...
---Create entry maker for group picker
---@param group table Group data with name, marker_count, etc.
---@return table entry Telescope entry with display, value, ordinal
local function make_group_entry(group)
  local display = string.format("%-20s %3d markers  %s", 
    group.name, 
    group.marker_count,
    relative_time(group.modified_at))
...
---Create entry maker for marker picker
---@param marker table Marker data with buffer_path, line info, annotation
---@return table entry Telescope entry with display, value, ordinal
local function make_marker_entry(marker)
  ...
  local display = string.format("%-25s:%4s  %s", 
    file_name, 
    tostring(line_info),
    annotation)
...
---Show Telescope picker for selecting marker groups
...
function M.show_groups(opts)
  ...
  -- Convert groups to array for telescope
  local group_array = {}
  ...
  -- Sort groups: active first, then by name
  ...
  pickers.new(opts, {
    ...
    previewer = previewers.new_buffer_previewer({
      ...
      define_preview = function(self, entry, status)
        ...
        -- Show first few markers if any exist
        if group.marker_count > 0 and group_data and group_data.markers then
          ...
          for i = 1, marker_count do
            ...
            if marker then
              ...
              table.insert(preview_text, 
                string.format("  %d. %s:%s - %s", i, file_name, line_info, 
                  string.sub(marker.annotation, 1, 30)))
            end
          end
      ...
      actions.select_default:replace(function()
        ...
        if selection then
          ...
          if result.success then
            feedback.success("Telescope Groups", 
...
---Show Telescope picker for selecting markers in the active group
---@param opts table? Optional picker configuration
  ...
  pickers.new(opts, {
    ...
    previewer = previewers.new_buffer_previewer({
      ...
      define_preview = function(self, entry, status)
        ...
        -- Try to get from loaded buffer first
        ...
        -- If not in buffer, try to read from disk
        ...
        if file_extension then
          -- Use the floating window's filetype detection
          local floating = require("marker-groups.ui.floating")
          if floating and floating.get_filetype_from_path then
            local filetype = floating.get_filetype_from_path(marker.buffer_path)
            ...
            local ft_map = {
              ...
              c = "c", cpp = "cpp", java = "java", php = "php",
              sh = "bash", vim = "vim", json = "json", yaml = "yaml",
        ...
        local preview_text = {
          ...
          "📏 Lines: " .. marker.start_line .. (marker.start_line ~= marker.end_line and " - " .. marker.end_line or ""),
        ...
        if file_content_available and #file_lines > 0 then
          ...
          -- Add context lines with proper formatting
          for i = context_start, context_end do
            ...
            -- Truncate very long lines for preview
      ...
      actions.select_default:replace(function()
        ...
        if selection then
          ...
          if marker.end_line and marker.end_line > marker.start_line then
            vim.cmd("normal! V")
            vim.api.nvim_win_set_cursor(0, { marker.end_line, 0 })
          end
          
          local file_name = vim.fn.fnamemodify(marker.buffer_path, ":t")
          feedback.success("Telescope Markers", 
