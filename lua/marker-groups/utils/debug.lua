---@class MarkerGroupsDebug
---Debug utilities and commands for marker-groups.nvim development and troubleshooting.
---Provides state inspection, debugging commands, and diagnostic tools.
local M = {}
...
---@param enable boolean? Enable debug mode (nil to toggle)
---@return boolean current_state Current debug mode state
function M.set_debug_mode(enable)
  ...
  -- Also set logger to debug level if enabling debug mode
  if enable then
...
function M.dump_state()
  ...
  dump.performance.config_access_100x = (end_time - start_time) / 1000000 -- Convert to ms
  
  return dump
end

---Print formatted state dump to messages
function M.show_state()
  ...
  for group_name, group_info in pairs(dump.state.groups_summary) do
    ...
    table.insert(lines, string.format("  • %s: %d markers%s", 
      group_name, group_info.marker_count, status))
  end
  ...
  table.insert(lines, "  • Neovim: " .. dump.neovim_version.major .. "." .. dump.neovim_version.minor .. "." .. dump.neovim_version.patch)
...
---@param filepath string? Custom file path (nil for auto-generated)
---@return string filepath Path where dump was written
...
---Inspect a specific marker group
---@param group_name string? Group name (nil for active group)
function M.inspect_group(group_name)
  ...
  local lines = {
    ...
    "  • Modified: " .. (group_data.modified_at and os.date("%Y-%m-%d %H:%M:%S", group_data.modified_at) or "unknown"),
  ...
  if group_data.markers and #group_data.markers > 0 then
    ...
    for i, marker in ipairs(group_data.markers) do
      ...
      table.insert(lines, string.format("  %d. %s:%s - %s", 
        i, file_name, line_info, marker.annotation or "no annotation"))
    end
  else
...
---Validate plugin state integrity
---@return table validation_result Validation results with issues found
...
function M.register_commands()
  -- Debug mode toggle
  vim.api.nvim_create_user_command("MarkerGroupsDebugMode", function(args)
    ...
    elseif arg == "" then
      ...
      vim.notify("Usage: :MarkerGroupsDebugMode [on|off]", vim.log.levels.ERROR)
      return
    end
    
    local new_state = M.set_debug_mode(enable)
    vim.notify("Debug mode: " .. (new_state and "ON" or "OFF"), vim.log.levels.INFO)
  end, {
    nargs = "?",
    desc = "Toggle or set debug mode",
    complete = function()
      return { "on", "off" }
    end
  })
  ...
  -- Show state
  vim.api.nvim_create_user_command("MarkerGroupsDebugState", function()
    M.show_state()
  end, {
    desc = "Show comprehensive plugin state"
  })
  
  -- Write state dump
  vim.api.nvim_create_user_command("MarkerGroupsDebugDump", function(args)
    local filepath = args.args ~= "" and args.args or nil
    M.write_state_dump(filepath)
  end, {
    nargs = "?",
    desc = "Write state dump to file",
    complete = "file"
  })
  
  -- Inspect group
  vim.api.nvim_create_user_command("MarkerGroupsDebugGroup", function(args)
    local group_name = args.args ~= "" and args.args or nil
    M.inspect_group(group_name)
  end, {
    nargs = "?",
    desc = "Inspect a specific group (current if no name given)",
    complete = function()
  ...
  -- Validate state
  vim.api.nvim_create_user_command("MarkerGroupsDebugValidate", function()
    M.show_validation()
  end, {
    desc = "Validate plugin state integrity"
  })
  
  -- Memory info
  vim.api.nvim_create_user_command("MarkerGroupsDebugMemory", function()
    ...
    local lines = {
      ...
      "═══════════════════",
      "",
      "Before GC: " .. string.format("%.2f KB", before),
      "After GC: " .. string.format("%.2f KB", after),
      "Freed: " .. string.format("%.2f KB", before - after),
      "",
      "Current Lua memory: " .. string.format("%.2f KB", collectgarbage("count"))
    }
    
    vim.notify(table.concat(lines, "\n"), vim.log.levels.INFO)
    logger.debug("Memory information displayed")
  end, {
