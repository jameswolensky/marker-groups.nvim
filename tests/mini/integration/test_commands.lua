...
T['group management / MarkerGroupsRename truncates long arg names'] = function()
  with_child(function(child)
    ...
    local has_trunc = child.lua([[local s=require('marker-groups.state'); local names=s.get_group_names(); local t=string.rep('x',100); return vim.tbl_contains(names,t)]])
...
T['marker commands / range add via ex-range'] = function()
  with_child(function(child)
    ...
    local m = child.lua([=[local m=require('marker-groups.markers'); local list=m.get_current_buffer_markers(); return list[#list]]=])
