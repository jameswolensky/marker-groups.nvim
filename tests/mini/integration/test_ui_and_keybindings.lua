...
T['drawer config / defaults and validation'] = function()
  with_child(function(child)
    ...
    local has_drawer = child.lua([[local cfg=require('marker-groups.config').get(); return type(cfg.drawer_config)=='table' and type(cfg.drawer_config.width)=='number' and type(cfg.drawer_config.side)=='string']])
...
T['drawer width / functions and clamping'] = function()
  with_child(function(child)
    ...
    local width_fn = child.lua([[local d=require('marker-groups.ui.drawer'); return type(d.set_drawer_width)=='function' and type(d.get_drawer_width)=='function']])
    ...
    local clamped_low = child.lua([[local d=require('marker-groups.ui.drawer'); d.set_drawer_width(20); return d.get_drawer_width() >= 30]])
    local clamped_high = child.lua([[local d=require('marker-groups.ui.drawer'); d.set_drawer_width(150); return d.get_drawer_width() <= 120]])
...
T['keymaps / setup loads and no bare <leader>m'] = function()
  with_child(function(child)
    ...
    local bare = child.lua([[for _,m in ipairs(vim.api.nvim_get_keymap('n')) do if m.lhs=='<leader>m' then return true end end; return false]])
