...
local T = MiniTest.new_set({
  hooks = {
    pre_case = function()
      ...
      require('marker-groups').setup({ data_dir = vim.fn.tempname() .. '_mg_eh', log_level = 'debug', keymaps = { enabled = false } })
    end,
  },
})
...
T['utilities / safe_execute success'] = function()
  local eh = require('marker-groups.error_handling')
  local res = eh.safe_execute('op', function() return { success = true, value = 'test' } end)
  expect_truthy(res.success)
...
T['utilities / safe_execute with errors'] = function()
  local eh = require('marker-groups.error_handling')
  local res = eh.safe_execute('op', function() error('boom') end)
