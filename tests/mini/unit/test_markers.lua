...
local expect_matches = MiniTest.new_expectation(
  'matches',
  function(str, pat) return type(str) == 'string' and string.find(str, pat) ~= nil end,
  function(str, pat) return string.format('Pattern %s not in %s', vim.inspect(pat), vim.inspect(str)) end
)
