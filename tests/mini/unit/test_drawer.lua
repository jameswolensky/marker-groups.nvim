...
T['navigation functions / should handle navigation with empty marker list'] = function()
  local drawer = require('marker-groups.ui.drawer')
  local empty = {}
  MiniTest.expect.no_error(function() drawer.navigate_to_next_marker(1, empty, 'down') end)
  MiniTest.expect.no_error(function() drawer.navigate_to_next_marker(1, empty, 'up') end)
end
...
T['toggle functionality / should handle toggle when no windows are open'] = function()
  ...
  drawer.has_open_windows = function() return false end
  local called = false
  drawer.show_markers = function() called = true; return 1, 1 end
...
T['toggle functionality / should handle toggle when windows are open'] = function()
  ...
  drawer.has_open_windows = function() return true end
  local closed = false
  drawer.close_all = function() closed = true end
  drawer.toggle_drawer()
...
T['delete functionality / should handle delete when no marker is found'] = function()
  ...
  MiniTest.expect.no_error(function() drawer.delete_current_marker(999, empty_positions) end)
end
...
T['delete functionality / should find correct marker at cursor position'] = function()
  ...
  markers.delete_marker = function(id) called_with = id; return { success = true } end
...
T['delete functionality / should handle delete failure gracefully'] = function()
  ...
  local markers = require('marker-groups.markers')
  local original_delete = markers.delete_marker
  markers.delete_marker = function() return { success = false, error = 'fail' } end

  MiniTest.expect.no_error(function() drawer.delete_current_marker(1000, marker_positions) end)
