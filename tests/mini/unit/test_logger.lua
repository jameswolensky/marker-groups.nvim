...
T['logging functions / debug, info, warn, error'] = function()
  ...
  for _, line in ipairs(logs) do
    if string.match(line, 'DEBUG.*Test debug message') then found.debug = true end
    if string.match(line, 'INFO.*Test info message') then found.info = true end
    if string.match(line, 'WARN.*Test warning message') then found.warn = true end
    if string.match(line, 'ERROR.*Test error message') then found.error = true end
  end
...
T['log filtering / respect level'] = function()
  ...
  for _, line in ipairs(logs) do
    if string.match(line, 'DEBUG') then debug_found = true end
    if string.match(line, 'INFO') then info_found = true end
    if string.match(line, 'WARN') then warn_found = true end
    if string.match(line, 'ERROR') then error_found = true end
  end
...
T['buffer management / clear preserves header and limits size'] = function()
  ...
  for _, line in ipairs(logs) do
    if string.match(line, 'Marker Groups Plugin Log') then has_header = true; break end
  end
  expect_truthy(has_header)
  logger.set_level('debug')
  logger.clear()
  for i = 1, 1100 do logger.debug('Test message ' .. i) end
